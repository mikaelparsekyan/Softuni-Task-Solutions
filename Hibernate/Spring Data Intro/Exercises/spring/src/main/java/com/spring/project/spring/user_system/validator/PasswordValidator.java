...
public class PasswordValidator implements ConstraintValidator<Password,CharSequence> {
    ...
    public boolean isValid(final CharSequence value, final ConstraintValidatorContext context) {
        ...
        if (value.length() > this.maxLength) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("Password too long")
                    .addConstraintViolation();
            return false;
        }
        ...
        if (!PATTERN_LOWER.matcher(password).find() && this.hasLower) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("Password not contain lower symbol")
                    .addConstraintViolation();
        ...
        if (!PATTERN_UPPER.matcher(password).find() && this.hasUpper) {
            context.buildConstraintViolationWithTemplate("Password not contain upper symbol")
                    .addConstraintViolation();
            return false;
        }

        if (!PATTERN_DIGIT.matcher(password).find() && this.hasDigit) {
            context.buildConstraintViolationWithTemplate("Password not contain digit")
                    .addConstraintViolation();
            return false;
        }

        if (!PATTERN_SYMBOL.matcher(password).find() && this.hasSpecialSymbol) {
            context.buildConstraintViolationWithTemplate( "Password not contain special symbol")
                    .addConstraintViolation();
            return false;
        }
